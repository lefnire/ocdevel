...
function Resource({node}: { node: Resource }) {
  ...
  const renderIcon = (filterKey: FilterKey) => {
    const id = `${filterKey}-${full[filterKey]}`
...
export function ResourceNode({node, level = 0}: { node: Resource | undefined, level?: number }) { // Allow node to be undefined initially
  const {flat} = useContext(ResourceCacheContext);
  if (!flat[node?.id]) {return null}
  return <ResourceNode_ node={node} level={level}/>
}

function ResourceNode_({node, level = 0}: { node: Resource, level: number }) { // node is guaranteed here by ResourceNode wrapper
  const {flat} = useContext(ResourceCacheContext);
  const full = flat[node.id]
